...
/**
 * @returns {string} absolute path to the opencode plugin source DIRECTORY
 *   (meko-memory.mjs plus its sibling exchange-helpers.mjs — meko-memory.mjs
 *   imports that file by relative path, so the whole directory must be
 *   copied together, not just the entry file. Mirrors resolveHooksRoot's
 *   directory-not-file contract for the same reason hooks-handlers/ is
 *   copied whole). Lives under skills/opencode-plugin/ in source, mirrored
 *   under assets/opencode-plugin/ in the npm-packed install.
 */
export function resolveOpencodePluginSource() {
  return join(getAssetRoot(), "opencode-plugin");
}
