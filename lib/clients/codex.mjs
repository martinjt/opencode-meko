 ...
 * @param {{name?: string, skillPath?: string|null}} [opts] - `skillPath`
 *   defaults to Codex's `~/.agents/skills/meko-mcp-tools/SKILL.md` (the
 *   skill file this installer actually places there for Codex). Pass an
 *   explicit `null` for clients that don't install that skill directory
 *   (e.g. opencode) so the block doesn't reference a path that doesn't exist.
 */
export function upsertAgentsMdBlock(filePath, version, opts = {}) {
  const name = opts.name ?? "meko";
  const skillPath = Object.prototype.hasOwnProperty.call(opts, "skillPath")
    ? opts.skillPath
    : "~/.agents/skills/meko-mcp-tools/SKILL.md";
  const block = renderAgentsMdBlock(version, name, skillPath);
...
function renderAgentsMdBlock(version, name, skillPath) {
  const toolPrefix = `mcp__${name}__`;
  const skillHint = skillPath ? ` Skill details: ${skillPath}` : "";
  return [
    `${AGENTS_MD_START} v${version}`,
    "## Meko memory (auto-managed by meko installer)",
    "",
    `You have a persistent memory store via the \`${name}\` MCP server. At the start of every conversation, call \`${toolPrefix}memory_search\` for relevant context. When the user shares facts, preferences, or decisions worth remembering, call \`${toolPrefix}memory_add\`.${skillHint}`,
    "",
