...
export function createOpencodeAdapter(customPaths = {}) {
  ...
  return {
    ...
    installSkills({ name, dryRun, log, scope, projectRoot }) {
      ...
      // opencode has no skills directory (unlike Codex, which this block's
      // text is shared with) — omit the "Skill details: ..." line rather
      // than pointing at a path this adapter never installs.
      upsertAgentsMdBlock(agentsMdPath, currentVersion, { name: serverName, skillPath: null });
    ...
    setHookEnvironment({ url, apiKey, dryRun, verbose, log, scope, projectRoot }) {
      ...
      // Copy the whole source directory (not just installPath's file) — the
      // plugin's sibling exchange-helpers.mjs is a relative import that must
      // land next to it. See resolveOpencodePluginSource's doc comment.
      installStableTree(resolvePluginSource(), dirname(installPath));
