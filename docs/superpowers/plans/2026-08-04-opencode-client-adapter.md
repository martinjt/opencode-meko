...
- [x] **Step 4: Have a real conversation and confirm capture worked** — **partially done, with a real bug found and fixed.** A `MEKO_API_KEY` became available (`~/repos/martinjt/.envrc`, targeting Cloud Meko at `https://mcp.mekodata.ai/mcp`). Installed against it in an isolated scratch project and ran a real `opencode run` session.

  Session-priming is now **confirmed working end-to-end against real Cloud Meko**: `conversation_get` on the ID our plugin's `createConversation` call returned shows a fully persisted, correctly-tagged conversation (`agent_id: "opencode:<repo-basename>"`, title `"opencode session (auto-captured)"`, our metadata) — proving `mcpInitialize`/`createConversation`/`fetchRecentMemories` and the `session.created` → `primeSession` wiring all work for real, not just in mocked unit tests. (Aside: `conversation_list` for that same agent_id returns empty even though `conversation_get` by ID finds it — apparently a Meko-server-side listing behavior, most likely filtering on `message_count > 0`; not something in this adapter's control.)

  Message-level capture (`session.idle` → `captureNewMessages` → `client.session.messages()` → `addMessage`) is **still unverified** — the LLM completion never finished in this environment (the user's self-hosted litellm endpoint at `llm.my2cents.co.uk` accepted the request but hung indefinitely past "llm runtime selected" on every attempt, confirmed unrelated to this adapter: the endpoint itself responds in 26ms to an unauthenticated request). `session.idle` never fires without a completed turn, so the exact response shape of `client.session.messages()` (`response.data` vs a bare array — the plugin code defensively handles both) remains the one unconfirmed inference in the whole design. Do this once the litellm endpoint issue is resolved or a different model/provider is available to complete a turn.

  **Found and fixed a second real bug in the process** (see commit `de9d4e5`): `resolveOpencodePluginSource()` pointed at the single `meko-memory.mjs` file, so the install-time copy never brought along its sibling `exchange-helpers.mjs` (a relative import). The plugin failed to load at runtime with **no error surfaced anywhere** — not in `opencode mcp list`'s shape-check, not in `opencode run`'s logs. This was only caught by manually instrumenting a scratch-installed copy with file-based debug logging (`console.warn`/`console.log` output from inside a plugin hook does not appear to surface in opencode's structured logs or `--print-logs` output at all — worth remembering for any future opencode-plugin debugging). Fixed by making `resolveOpencodePluginSource()` return the plugin asset directory (mirroring `resolveHooksRoot`'s existing directory-not-file contract) so `installStableTree` copies both files together. Regression tests added in both `test/opencode-config.test.mjs` and `test/opencode-client.test.mjs`.
...
All three fixes (the two from the first smoke-test pass, plus the sibling-import copy bug from the follow-up real-capture pass) are committed with tests. What's proven end-to-end against real Cloud Meko: MCP registration, plugin loading, session-priming, and conversation creation. What remains unverified: message-level capture specifically (`session.idle` → `client.session.messages()` → `addMessage`), blocked on an unrelated LLM-completion hang in this environment, not a known or suspected bug in this adapter.
